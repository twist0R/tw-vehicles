function moneyRemover(src, amount)
    local user = exports["tw-base"]:getModule("Player"):GetUser(src)
...
AddEventHandler("transfer:attempt:send", function(plate, target)
    local pSrc = source
    local pUser = exports["tw-base"]:getModule("Player"):GetUser(pSrc)
    local pChar = pUser:getCurrentCharacter().id
    local pSteam = GetPlayerIdentifiers(pSrc)[1]

    -- target shit
    local tUser = exports["tw-base"]:getModule("Player"):GetUser(target)
